...
import numpy as np
import sympy
from esr.fitting import combine_DL, match, plot, test_all, test_all_Fisher
...
def make_data(name, f, x_range, nx, frac_sigx, samp_num, sig, make_fig=False):
    ...
    if make_fig:
        ...
        plt.errorbar(xdata, ydata, yerr=sig, **plot_kwargs)

        plt.xlabel(r"$x$")
        plt.ylabel(r"$y$")
        plt.title(name + ": " + f"${sympy.latex(expr)}$")
        plt.tight_layout()
    ...
    np.savetxt(
        f"../data/{fname}.txt",
        np.vstack([xdata, ydata, sig * np.ones(len(xdata))]).transpose(),
    )
...
class MockLikelihood:
    ...
    def negloglike(self, a, eq_numpy, **kwargs):
        ...
        if not np.all(np.isreal(y)):
            return np.inf
        nll = np.sum(
            0.5 * (y - self.yvar) ** 2 * self.inv_cov
...
def apply_language_prior(likelihood, comp, tmax=5):
    ...
    likelihood.fnprior_prefix = "katz_codelen_2_"
    ...
    likelihood.fnprior_prefix = "katz_logprior_2_"
...
def process_data(dirname, final_prefix, all_comp):
    ...
    for i, compl in enumerate(all_comp):
        fname = dirname + final_prefix + f"{compl}.dat"
...
def _process_fit(
    name, all_true_eq, nx, frac_sigx, samp_num, all_comp, remove_by_like=True
):
    """
...
def process_fit(dirname, all_comp, nx, all_true_eq=None, remove_by_like=True):
    ...
    fun, loss, _ = remove_duplicates_and_sort(
        fun, res[:, 2], res[:, 2], remove_by_like=remove_by_like
    )
    ...
    fun, loss, _ = remove_duplicates_and_sort(
        fun, res[:, 0], res[:, 2], remove_by_like=remove_by_like
    )
    ...
    if m4_ftrue not in m4_fun[:2]:
        print("\nMethod 4", dirname, m4_fun[: min(len(m4_fun), 4)])
    ...
    loss = (
        ...
        - p / 2 * np.log(b)
        + res[:, 4]
        + p / 2 * np.log(2 * np.pi * nup)
    )
    m5_ftrue, m5_ltrue = get_truth_location(fun, loss, all_true_eq)
    fun, loss, _ = remove_duplicates_and_sort(
        fun, loss, res[:, 2], remove_by_like=remove_by_like
    )
    ...
    fun, res, params, store_comp = process_data(
        dirname, "final_noconst_katz_2_", all_comp
    )
    m = params != 0
    p = np.sum(m, axis=1)
    ...
    with open(dirname + "/selection_summary.csv", "w") as f:
        ...
        if all_true_eq is None:
            writer.writerow(
                ["Method"]
                + [f"f{i}" for i in range(nkeep)]
                + [f"loss{i}" for i in range(nkeep)]
            )
...
def main():
    ...
    all_true_eq = {
        ...
        "korns_7": [
            ...
            "a0*(a1 - pow(Abs(a2),x))",
            "a0 + a1*pow(Abs(a2),x)",
            "a0 + a1*pow(Abs(a2),(2*x))",
            "a0 - pow(Abs(a1),x)/sqrt(Abs(a2))",
            "(a0 - pow(Abs(a1),x))/sqrt(Abs(a2))",
            "a0 - pow(Abs(a1),(x/2))*sqrt(Abs(1/a2))",
            "a0*(cos(a1) - pow(Abs(a2),x))",
            "a0 - pow(Abs(a1),x)/cos(a2)",
            "(a0 - pow(Abs(a1),x))/cos(a2)",
        ...
        for frac_sigx in all_sigx:
            ...
            if do_make_mocks:
                for nx, samp_num in combo[data_start:data_end]:
                    make_data(
                        name, f, x_range, nx, frac_sigx, samp_num, sig, make_fig=False
                    )
            comm.Barrier()
            ...
            if do_fit_mocks:
                for nx, samp_num in combo:
                    for comp in all_comp:
                        print_text(
                            f"FITTING {name}, {frac_sigx}, {nx}, {samp_num}, {comp}"
                        )
            ...
            if do_language_model:
                for nx, samp_num in combo:
                    for comp in all_comp:
                        print_text(
                            f"LANGUAGE {name}, {frac_sigx}, {nx}, {samp_num}, {comp}"
                        )
